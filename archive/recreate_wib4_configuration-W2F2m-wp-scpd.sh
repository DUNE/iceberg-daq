...
wibconf_gen -f -c $HERE/iceberg_wib4.json $CONF_DIR/iceberg_wib_conf >> $HERE/../logs/iceberg_wib_conf.log
#wibconf_gen -f -c $HERE/iceberg_wib3.json $CONF_DIR/iceberg_wib_conf >> $HERE/../logs/iceberg_wib_conf.log
...
if [ "$do_wibpulser" = 1 ]; then
    : sed -i '/femb[2]/,/enabled/{s/true/false/}
           /enabled_[2]/       {s/true/false/}' $CONF_DIR/iceberg_wib_conf/data/wib102_conf.json
    : sed -i '/femb[23]/,/enabled/{s/true/false/}
            /enabled_[23]/      {s/true/false/}' $CONF_DIR/iceberg_wib_conf/data/wib103_conf.json
else
    sed -i '/femb[2]/,/enabled/{s/true/false/}' $CONF_DIR/iceberg_wib_conf/data/wib104_conf.json
    : sed -i '/femb[23]/,/enabled/{s/true/false/}' $CONF_DIR/iceberg_wib_conf/data/wib103_conf.json
fi
